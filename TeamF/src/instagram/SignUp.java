...
class SignUp {
	 public SignUp(){
	     ...
	     ok.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int error=-1;
				String[] list = sql.getUser_Id();
				for(int i=0;i<list.length;i++) {
					if(user_tf.getText()==list[i]){
				...
				else if(nickname.equals("")&&user.equals("")&&name.equals("")&&password.equals("")&&birth.equals("")&&Address.equals("")&&phone.equals("")) {
					JOptionPane.showMessageDialog(null, "Fill the blank");
				}
				else {
					UserStruct newOne = new UserStruct(user,name,password,birth,gender,phone,Address);
					sql.createUser(newOne, nickname);
					LogIn login = new LogIn();
					login.setVisible(true);
					frame.setVisible(false);
				}
